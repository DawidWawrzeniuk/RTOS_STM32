...
typedef struct tcb tcbType;	//o oznacza: „Od teraz tcbType jest równoważne struct tcb”.


tcbType tcbs[NUM_OF_THREADS];
tcbType *currentPt;

//Each thread will have stacksize of 100 (400bytes)
...
void osKernelStackInit(int i)
{
	...
	TCB_STACK[i][STACKSIZE -1] = (1U<<24); 		//PSR register (thumb mode)
...
uint8_t osKernelAddThreads(void(*task0)(void),void(*task1)(void),void(*task2)(void))	//void(*task0)(void) to deklaracja wskaźnika na funkcję					void — funkcja nic nie zwraca			(*task0) — task0 jest wskaźnikiem			(void) — funkcja nie przyjmuje argumentów
{
	...
	osKernelStackInit(0);
	//Initialize PC
	TCB_STACK[0][STACKSIZE - 2] = (int32_t)(task0);		//Włóż adres funkcji task0 na stos wątku 0 w miejsce, gdzie normalnie znajduje się PC
...
//When exception occurs these registers are automaticly saved onto the stack:
...
__attribute__((naked)) void SysTick_Handler(void)
{
	...
	__asm("LDR R0, =currentPt");	//zaladuj do r0 adres zmiennej currentPt
	//Load r1 from address equals r0, i.e. r1=currentPt
	__asm("LDR R1,[R0]");		//wczytaj do r1 wartosc z pamieci pod adresem zapisanym w r0  (czyli adres TCB aktualnego wątku)
	//Store Cortex-M SP at address equals r1, i.e. Save SP into tcb
	__asm("STR SP,[R1]");	//zapisz wartosc SP pod adres wskazywany przez R1

	//Choose next thread
	__asm("PUSH {R0,LR}");						//bo BL nadpisuje te rejestry to musimy je wrzucic na stos
...
{
	__disable_irq();							//chodzi o ochronę przed jednoczesnym dostępem do tej samej zmiennej (tzw. race condition)
	*semaphore += 1;
	__enable_irq();
}
